...
(async () => {
	try {
		await replace({
			...
			from: /github.com\/ptkdev-boilerplate\/svelte-spa-boilerplate/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /github.com\/ptkdev\/svelte-spa-boilerplate/g,
			to: setup.github_full_repository_url,
		});
	} catch (error) {
		logger.error(JSON.stringify(error));
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /@ptkdev\/svelte-spa-boilerplate/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /Create single page application with this user-friendly Svelte boilerplate made with typescript and love/g,
	...
	try {
		await replace({
			...
			from: /"projectName": "ptkdev-boilerplate\/svelte-spa-boilerplate"/g,
